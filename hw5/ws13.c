 ...
 *
 *  Changed initialization in for loop to be to the link pointer
 *  in the dummy head node, not the first node to which the dummy
 *  head points to.
 */
...
delete( symtab_t *tp, char str[] )
{
	...
	//change tp->head.next to tp->head
	for( linkp = &tp->head ; linkp ; linkp = linkp->next )
...
void clear_table(symtab_t *tp)
{
	...
	struct link *lp = tp->head.next;  //get first node
	struct link *one_to_delete;       //temp pointer
